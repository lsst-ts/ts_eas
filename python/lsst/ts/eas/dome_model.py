...
class DomeModel:
    """A model for MTDome open/closed state.

    Track whether and when the dome has been opened, so that other models can
    gate their behavior on it. Louver positioning lives in `LouverModel`; this
    model only observes telemetry and never commands MTDome.
