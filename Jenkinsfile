    ...
    stages {
        ...
        stage("Build IDL files") {
            steps {
                script {
                    sh """
                    docker exec -u saluser \${container_name} sh -c \"source ~/.setup.sh && setup ts_sal -t current && make_idl_files.py EAS\"
                    """
                }
            }
        }
        ...
        stage("Running tests") {
            steps {
                script {
                    sh """
                    docker exec -u saluser \${container_name} sh -c \"source ~/.setup.sh && cd repo && pip install --ignore-installed -e . && eups declare -r . -t saluser && setup ts_eas -t saluser && pytest --junitxml=\${XML_REPORT}\"
                    """
                }
            }
        }
        ...
        always {
            ...
            script {
                def RESULT = sh returnStatus: true, script: "docker exec -u saluser \${container_name} sh -c \"" +
                    ...
                    "cd /home/saluser/repo/ && " +
                    "setup ts_eas -t saluser && " +
                    "ltd upload --product ts-eas --git-ref \${GIT_BRANCH} --dir doc/_build/html\""
